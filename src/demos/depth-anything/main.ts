/**
 * Depth-anything page controller. Two independent run modes share this page,
 * mirroring object-detection: the discrete N-backend compare grid for a
 * single image, and a live single-backend loop over a camera feed or an
 * uploaded video file. One live code path covers both sources, because a
 * played-back video's `captureStream()` and a webcam's `getUserMedia()` both
 * hand back a plain MediaStreamTrack.
 */
...
import {DepthAnythingLiveStage} from './stage-live';
...
const controller = createCompareController({
  ...
  logger: liveLogger,
  createStage: (canvas) => ({
    stage: new DepthAnythingStage(canvas),
    container: canvas,
  }),
...
const liveStage = new DepthAnythingLiveStage(liveCanvas);
let live = false;
...
function applyInputMode(mode: InputMode): void {
  ...
  liveToggleButton.textContent = mode === 'camera' ? 'Start Camera' : 'Start Depth';
...
async function stopLive(): Promise<void> {
  ...
  sourceVideo.pause();
  live = false;
  liveToggleButton.textContent = currentInputMode() === 'camera' ? 'Start Camera' : 'Start Depth';
