...
export function renderEfficientVitLive(
  ...
  if (!output) throw new Error('efficientvit-seg live: model declares no outputs');

  const values = data[output.name];
  if (!values) throw new Error(`efficientvit-seg live: no output data for "${output.name}"`);
  ...
  if (outHeight === undefined || outWidth === undefined || numClasses === undefined) {
    throw new Error(`efficientvit-seg live: unexpected output rank ${shape.length}, shape [${shape}]`);
  }
